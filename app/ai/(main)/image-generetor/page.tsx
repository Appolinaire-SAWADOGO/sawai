...
import { imageGenerateType } from "@/lib/type/data-type";
...
const Page = () => {
  ...
  const { data } = useQuery<imageGenerateType>({
    queryKey: ["get-content-data"],
    queryFn: async () =>
      axios.get("/api/get-content-data").then((resp) => resp.data),
  });
  ...
  const { mutate: onSubmit, isLoading } = useMutation({
    mutationFn: async () => {
      ...
      const data = await axios
        .post("/api/image-generator", { prompt: prompt })
        .then((resp) => {
    ...
    onSuccess: async () => {
      router.refresh();
      await queryClient.fetchInfiniteQuery("get-content-data");
            ...
            {data?.map((data, index) => (
              ...
              >
                ...
                {data.who === "YOU" ? (
                  ...
                  <div className="flex h-[400px] xl:w-4/5  w-full rounded-lg mt-8 relative">
                    {isLoaded ? (
                      <>
                        <Image
                          src={data.imageUrl}
                        ...
                        />
                        <div className="flex items-center justify-center cursor-pointer hover:bg-opacity-70 w-12 h-12 absolute top-2 left-3 rounded-full bg-[#27282A]">
                          <a
                            href={data.imageUrl}
                            download
                            aria-label="Download Audio"
                          >
                            <ArrowDownToLine className="w-6 h-6 text-color" />
                          </a>
                        </div>
                      </>
                    ) : (
                      <Skeleton className="w-full h-full bg-[#d8d8d8] bg-opacity-25" />
                    )}
                  </div>
                )}
              </div>
