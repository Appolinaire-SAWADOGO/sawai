...
import { contentGenerateType } from "@/lib/type/data-type";
...
const Page = () => {
  ...
  const { data } = useQuery<contentGenerateType>({
  ...
  const { mutate: onSubmit, isLoading } = useMutation({
    mutationFn: async () => {
      ...
      const data = await axios
        .post("/api/content-generator", { prompt: prompt })
        .then((resp) => {
          ...
          <div className="flex flex-col gap-10 w-full">
            {data?.map((data, index) => (
              ...
              >
                ...
                <p className="break-words whitespace-normal w-ful mt-4 text-color opacity-70">
                  {data.content}
                </p>
              </div>
            ))}
